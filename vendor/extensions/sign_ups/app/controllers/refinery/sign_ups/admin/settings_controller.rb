module Refinery
  module SignUps
    module Admin
      class SettingsController < Refinery::AdminController
        ...
        def update
          ...
          if @setting.update_attributes(params[:setting])
            flash[:notice] = t('refinery.crudify.updated', :what => @setting.name.gsub("sign_up_", "").titleize)

            unless request.xhr? or from_dialog?
              redirect_back_or_default(refinery.sign_ups_admin_sign_ups_path)
            else
              render :text => "<script type='text/javascript'>parent.window.location = '#{refinery.sign_ups_admin_sign_ups_path}';</script>"
            end
          end
        end
      ...
      protected
        def check_setting
          setting = params[:id].gsub("sign_up_", "")
